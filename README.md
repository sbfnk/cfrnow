...
`fit_cfr()` is a thin wrapper over `epidist::epidist()`. It takes the
prepared data, a `brms` formula (default `mu ~ 1` for the delay
location), the delay `family` (`lognormal()` or `Gamma()`), and `brms`
priors — the default is a weakly-informative logit-normal on the CFR:
...
Because the model is an `epidist` subclass, covariates go straight on
the CFR (or the delay) through the formula — this is how you get a
stratified or, with a smooth term on time, a time-varying CFR:
...
(it reduces to the naive ratio with the delay as a nuisance). To
estimate the delay from a line list first, use
[epidist](https://epidist.epinowcast.org/) directly and set an
informative prior on `mu`/`sigma`.
