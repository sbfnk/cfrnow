test_that("simulated delays match the requested mean and sd", {
  set.seed(1)
  specs <- list(distspec::LogNormal(mean = 12.75, sd = 7),
                distspec::Gamma(mean = 12.75, sd = 7))
  for (delay in specs) {
...
test_that("a recovery delay adds recoveries for non-fatal cases only", {
  ...
  no_rec <- simulate_linelist(n = 5, delay = distspec::LogNormal(2, 0.5))
...
test_that("unsupported families and prior-parameter delays are rejected", {
  ...
  expect_error(stan_delay_fields(distspec::Normal(mean = 5, sd = 1),
                                 "dist_id", "p"))
  expect_error(sample_delay(5, distspec::LogNormal(
    meanlog = distspec::Normal(2, 0.1), sdlog = distspec::Normal(0.5, 0.1))))
})

test_that("stan_delay_fields reads estimated (Normal) native parameters", {
  d <- distspec::LogNormal(meanlog = distspec::Normal(2.4, 0.2),
                            sdlog = distspec::Normal(0.5, 0.15))
...
test_that("parse_cfr_prior accepts a fixed Beta and rejects everything else", {
  expect_equal(parse_cfr_prior(distspec::Beta(6.6, 13.4)),
               c(a = 6.6, b = 13.4))
  expect_equal(parse_cfr_prior(distspec::Beta(mean = 0.1, sd = 0.1))[["a"]],
               0.1 * (0.1 * 0.9 / 0.1^2 - 1))
  expect_error(parse_cfr_prior(5), "distspec distribution")
  expect_error(parse_cfr_prior(distspec::Gamma(shape = 3, rate = 1)), "Beta")
  expect_error(
    parse_cfr_prior(distspec::Beta(shape1 = distspec::Normal(2, 0.1),
                                    shape2 = 5)),
    "fixed"
  )
})
...
test_that("stan_delay_fields reads a fixed delay (fixed-F), with a q prefix too", {
  d <- distspec::Gamma(shape = 3, rate = 0.24)
